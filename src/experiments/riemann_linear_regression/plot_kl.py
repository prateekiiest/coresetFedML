...
for i, nm in enumerate(nms):
    ...
    fig.patch(
        x=np.hstack((x, x[::-1])),
        y=np.hstack(
            (np.percentile(kl, 75, axis=0), np.percentile(kl, 25, axis=0)[::-1])
        ),
        color=pal[i],
        fill_alpha=0.4,
        legend=nm[1],
    ...
    if nm[0] == "IHT":
        fig.circle(
            x[::marker_plot_every],
            np.percentile(kl, 50, axis=0)[::marker_plot_every],
            fill_color=pal[i],
            size=marker_size,
            legend=nm[1],
        )
...
fig.legend.items = (
    fig.legend.items[legend_len - 2 : legend_len] + fig.legend.items[0 : legend_len - 2]
)
