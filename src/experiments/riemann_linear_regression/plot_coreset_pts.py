...
for trial_num in [1, 2, 3, 4]:
    ...
    fig = bkp.figure(x_range=lonbds, y_range=latbds,
    ...
    f.scatter(x[:, 1], x[:, 0], fill_color='black',
              size=12, alpha=0.01, line_color=None)
    ...
    for m in Ms:
        ...
        # plot data and coreset pts
        fig.scatter(x[:, 1], x[:, 0], fill_color='black',
                    size=12, alpha=0.01, line_color=None)
        ...
        fig.output_backend = 'svg'
        fig_name = 'exp2-contour_' + nm[0] + \
            '_m' + str(m) + '_id_' + str(trial_num)
        export_svgs(fig, filename=fig_name + '.svg')
        # cairosvg.svg2pdf(url=fig_name+'.svg', write_to=fig_name+'.pdf')
        cairosvg.svg2pdf(
            file_obj=open(fig_name + '.svg', "rb"), write_to=fig_name + '.pdf')
