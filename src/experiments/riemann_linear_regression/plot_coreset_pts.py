...
for trial_num in [1, 2, 3, 4]:
    ...
    contour_colors = [
        "#%02x%02x%02x" % (int(r), int(b), int(g))
        for (r, b, g) in zip(255 * c, 0 * np.ones(c.shape[0]), 255 * (1.0 - c))
    ]
    ...
    f.scatter(
        x[:, 1], x[:, 0], fill_color="black", size=12, alpha=0.01, line_color=None
    )
    # compute posterior mean regression on the grid
    reg = np.zeros(longrid.shape)
    for i in range(basis_scales.shape[0]):
        reg += mup[i] * np.exp(
            -((longrid - basis_locs[i, 1]) ** 2) / (2 * basis_scales[i] ** 2)
            - (latgrid - basis_locs[i, 0]) ** 2 / (2 * basis_scales[i] ** 2)
        )
    # contour_levels
    ...
    for color, level in zip(contour_colors, contour_levels):
        ...
        for contour in contours:
            ...
            latlons = np.hstack(
                (
                    np.interp(contour[:, 0], np.arange(lats.shape[0]), lats)[
                        :, np.newaxis
                    ],
                    np.interp(contour[:, 1], np.arange(lons.shape[0]), lons)[
                        :, np.newaxis
                    ],
                )
            )
    ...
    cairosvg.svg2pdf(file_obj=open(fig_name + ".svg", "rb"), write_to=fig_name + ".pdf")

    # coreset figures
    for m in Ms:
        fig = bkp.figure(
            x_range=lonbds, y_range=latbds, plot_width=1000, plot_height=1000
        )
        ...
        fig.scatter(
            ...
            x[:, 0],
            fill_color="black",
            size=30 * np.power(wt[m, :] / wt[m, :].max(), 0.4),
            line_color=None,
        )
        # compute posterior mean regression on the grid
        reg = np.zeros(longrid.shape)
        ...
        countour_legend = Label(
            ...
            x_units="screen",
            y_units="screen",
            text=nm[1] + " Corset Posterior " + "Coreset size: " + str(m),
            text_font_size="32pt",
        )
